...
```{asis, echo=is.xenofilt}
Summary of reads classified as host and removed is given in table below.
```



```{r summary-host-filt, eval=is.xenofilt}
...
```{asis, echo=is.xenofilt}
These data contain graft read pool.
```



```{r read-map-stats-data, eval=!is.xenofilt,include=FALSE,cache=FALSE}

```


```{r read-map-stats-data-xenofilt, eval=is.xenofilt, include=FALSE,cache=FALSE}
